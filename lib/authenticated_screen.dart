...
class _AuthenticatedScreenState extends State<AuthenticatedScreen> {
    ...
    if (data != null) {
      ...
      if (data.link.toString().contains("mangaId")) {
        ...
        ChapterModel chapterModel = ChapterModel(
          ...
          isPurchase: rawData[4].split("=").last=="null"|| rawData[4].split("=").last =="false" ? false:true,
          totalPages: 0,
          chapterNo: 0,
        );
        print(chapterModel.id.toString() + " chapterId");
        Navigator.pushNamed(context, Reader.routeName, arguments: [
          chapterModel,
          <ChapterModel>[],
          0,
          'asc',
  ...
  Future<void> initDynamicLinks() async {
    ...
    FirebaseDynamicLinks.instance.onLink(
        onSuccess: (PendingDynamicLinkData? dynamicLink) async {
          print(data!.link.toString() + "instance link");
  ...
  Widget build(BuildContext context) {
    return RoutesBottomNav(
        isAdmin: isAdmin == null ? widget.isAdmin : isAdmin);
  }
}
